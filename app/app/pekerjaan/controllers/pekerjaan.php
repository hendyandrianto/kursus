...
class Pekerjaan extends CI_Controller {
	...
	public function get_data(){
        ...
        $rResult = $this->pekerjaan_model->data($aColumns, $sWhere, $sOrder, $sLimit);
        $iFilteredTotal = 10;
        $rResultTotal = $this->pekerjaan_model->data_total($sIndexColumn);
	...
	public function add(){
		...
		$isi['actionedit'] = 'edit';
		$isi['halaman'] = "Data Pekerjaan";
		$isi['judul'] = "Halaman Add Data pekerjaan";
		$isi['content'] = "form_add";
		$this->load->view("dashboard/dashboard_view",$isi);		
	}
	public function proses_add(){
		$this->form_validation->set_rules('nama', 'Jenis Pekerjaan', 'htmlspecialchars|trim|required|xss_clean');
		if ($this->form_validation->run() == TRUE){
			$nama = $this->input->post('nama');
			$simpan = array('nama'=>$nama);
			$this->db->insert('tbl_pekerjaan',$simpan);
	...
	public function hapus($kode=Null){
		$ckdata = $this->db->get_where('tbl_member',array('id_pekerjaan'=>$kode))->result();
		if(count($ckdata)>0){
			$data['say'] = "NotOk";
		}else{
			$this->db->where('id',$kode);
			if($this->db->delete('tbl_pekerjaan')){
				$data['say'] = "ok";
			}else{
				$data['say'] = "NotOk";
			}
	...
	public function cekdata($kode=Null){
		$ckdata = $this->db->get_where('tbl_pekerjaan',array('id'=>$kode))->result();
		if(count($ckdata)>0){
			$data['say'] = "ok";
		}else{
			$data['say'] = "NotOk";
	...
	public function edit($kode=Null){
		...
		if(count($ckdata)>0){
			...
			$isi['judul'] = "Halaman Edit Data pekerjaan";
			$isi['content'] = "form_add";
			$isi['halaman'] = "Edit Data pekerjaan";
			$this->load->view("dashboard/dashboard_view",$isi);
		}else{
			redirect('error','refresh');
		}
	...
	public function proses_edit(){
		$this->form_validation->set_rules('nama', 'Data pekerjaan', 'htmlspecialchars|trim|required|min_length[1]|xss_clean');
		if ($this->form_validation->run() == TRUE){
			$nama = $this->input->post('nama');
