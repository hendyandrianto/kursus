...
$ckdata = $this->db->query("SELECT * FROM view_laporan WHERE LEFT(kode_trans,4) = 'MREG' AND LEFT(tgl_bayar,10) = '$tgl' AND id_tipe = '$tipe'")->result();
...
$i=0;
$bayartran = 0;
$ckdata = $this->db->query("SELECT * FROM view_laporan WHERE LEFT(kode_trans,4) = 'TRAN' AND LEFT(tgl_bayar,10) = '$tgl' AND id_tipe = '$tipe'")->result();
...
$i=0;
$bayarmig = 0;
$ckdata = $this->db->query("SELECT * FROM view_laporan WHERE LEFT(kode_trans,4) = 'MRAN' AND LEFT(tgl_bayar,10) = '$tgl' AND id_tipe = '$tipe'")->result();
...
$i=0;
$bayarmig = 0;
$ckdata = $this->db->query("SELECT * FROM view_laporan WHERE LEFT(kode_trans,4) = 'IREG' AND LEFT(tgl_bayar,10) = '$tgl' AND id_tipe = '$tipe'")->result();
foreach ($ckdata as $row2) {
    $i++;
    $bayarmig += $row2->bayar;
    $obj_pdf->MultiCell(8, 5, $i . ".", 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
...
$obj_pdf->Ln(5);
$total = 0;
$cksub = $this->db->query("SELECT * FROM view_laporan WHERE LEFT(tgl_bayar,10) = '$tgl' AND id_tipe = '$tipe'")->result();
